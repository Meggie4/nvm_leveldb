...
SETUP() {
  if [ -z "$TEST_TMPDIR" ]
  then
        echo "default DB path empty. Run source scripts/setvars.sh from source parent dir"
        exit
  fi
  rm -rf $TEST_TMPDIR/*
  mkdir -p $TEST_TMPDIR
  ...
  if [ -z "$TEST_MEMDIR" ]
  then
        echo "mem DB path empty. Run source scripts/setvars.sh from source parent dir"
        exit
  fi
  rm -rf $TEST_MEMDIR/*
  mkdir -p $TEST_MEMDIR
